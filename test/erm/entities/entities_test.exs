defmodule Erm.EntitiesTest do
  ...
  describe "tags" do
    ...
    test "list_tags/0 returns all tags" do
      tag = tag_fixture()
      assert Entities.list_tags() == [tag]
    end

    test "get_tag!/1 returns the tag with given id" do
      tag = tag_fixture()
      assert Entities.get_tag!(tag.id) == tag
    end

    test "create_tag/1 with valid data creates a tag" do
      assert {:ok, %Tag{} = tag} = Entities.create_tag(@valid_attrs)
    ...
    test "create_tag/1 with invalid data returns error changeset" do
      assert {:error, %Ecto.Changeset{}} = Entities.create_tag(@invalid_attrs)
    end

    test "update_tag/2 with valid data updates the tag" do
      tag = tag_fixture()
      assert {:ok, %Tag{} = tag} = Entities.update_tag(tag, @update_attrs)
    ...
    test "update_tag/2 with invalid data returns error changeset" do
      tag = tag_fixture()
      assert {:error, %Ecto.Changeset{}} = Entities.update_tag(tag, @invalid_attrs)
      assert tag == Entities.get_tag!(tag.id)
    end

    test "delete_tag/1 deletes the tag" do
      tag = tag_fixture()
      assert {:ok, %Tag{}} = Entities.delete_tag(tag)
      assert_raise Ecto.NoResultsError, fn -> Entities.get_tag!(tag.id) end
    end

    test "change_tag/1 returns a tag changeset" do
      tag = tag_fixture()
      assert %Ecto.Changeset{} = Entities.change_tag(tag)
    end
  end
end
