defmodule ErmWeb.RoleController do
  use ErmWeb, :controller

  alias Erm.Authorizations
  alias Erm.Authorizations.Role

  plug Guardian.Permissions.Bitwise, [ensure: %{roles: [:all]}]

  def index(conn, _params) do
    roles = Authorizations.list_roles()
  ...
  def create(conn, %{"role" => role_params}) do
    ...
    case Authorizations.create_role(role_params) do
      {:ok, role} ->
        conn
  ...
  def update(conn, %{"id" => id, "role" => role_params}) do
    ...
    case Authorizations.update_role(role, role_params) do
      {:ok, role} ->
        conn
  ...
  def delete(conn, %{"id" => id}) do
    role = Authorizations.get_role!(id)
    {:ok, _role} = Authorizations.delete_role(role)
